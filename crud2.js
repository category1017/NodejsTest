...
// 기본환경설정파일 env에 들어있는 port정보 또는 지정한 3000 웹서버 포트 생성
...
//body-parser form-urlencoding설정
app.use(bodyParser.urlencoded({extended:false}));
//body-parser json파싱설정
app.use(bodyParser.json());

// public 폴더를 static 콘텐츠 공간으로 설정
...
if(pool){
    pool.getConnection(function(err, conn){
        if(err){
            console.log("데이터베이스 pool 에러");
            if(conn){
                console.log("데이터베이스 conn 해제");
        ...
        conn.on('error',function(err){
            console.log('데이터베이스 연결시 에러가 발생했습니다.');
            console.dir(err);
            return;
        });
        ...
        pool.getConnection(function(err, conn){
            ...
            if(conn){
                ...
                var exec = conn.query("delete from users where id = ?", paramId, function(err, result){
                  conn.release();
                  console.log("디버그 : 삭제 쿼리 확인"+exec.sql);
                  if(err) {
                    ...
                    if(result.affectedRows >0){
                        res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
                        res.write('<script>alert("삭제 되었습니다.");location.replace("/");</script>');
                        res.end();
                    }else{
                        res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
        ...
        pool.getConnection(function(err, conn) {
            ...
            if(conn) {
                ...
                var exec = conn.query("update users set ? where id = ?",[updateSet, paramId],function(err, result) {
                    ...
                    console.log("디버그 update결과 : " + result.changedRows);//데이터가 실제로 변경된 행의 수
    ...
    if(pool){
        viewUser(req.query.id, function(err, result){
            ...
            if(result[0]){
                ...
                //jsonData = JSON.stringify(result[0]);//제이슨데이터를 스트링으로 변환-> 필요X
...
//아래 리스트를 '/' 라우터로 대체
//리스트 사용자페이지 연결 ->이 라우터를 사용하지 않고 ejs뷰템플릿을 사용
    ...
    if(pool){
        allUser(function(err,result){
            ...
            if(result){
                ...
                res.writeHead('200',{'Content-Type':"text/html;charset=utf8"});
                res.write('<h2>조회된 사용자 리스트 값이 없습니다.</h2>');
                res.write('<a href="/">이전화면으로 이동</a>');
                res.end();
            }
        });
    }else{
...
router.route('/process/adduser').post(function(req,res){
    ...
    //html에서 넘어온 데이터를 req로 받아서 처리 (아래)
    ...
    if(pool){
        addUser(paramId, paramName, paramAge,paramPassword, function(err, result){
            if(err){
                ...
                res.writeHead('200',{'Content-Type':"text/html;charset=utf8"});
                res.write('<h2>사용자 추가 중 에러가 발생했습니다. </h2>');
               
                res.write('<a href="/public/adduser2.html">이전화면으로 이동</a>');
                res.end();
            }
            if(result){
                console.dir(result);
                ...
                res.write('<br><a href="/">사용자 리스트</a>');
                res.write('<br><a href="/public/adduser2.html">사용자 등록</a>');
                res.end();
            }
        });
    }
...
var addUser = function(id, name, age, password, callback){
    console.log('addUser함수형 변수가 호출됨 : ');
    pool.getConnection(function(err, conn){
        if(err){
            if(conn){
        ...
        // html파라미터 데이터를 insert쿼리에 사용하기 위해서 객체로 만듦
    ...
    if(pool){
        authUser(paramId, paramPassword, function(err, result){
            ...
            if(result){
                ...
                res.write('<a href="/public/login2.html">다시 로그인</a>');
                res.end();
            }
        });
    }
